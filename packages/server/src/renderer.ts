...
export class Renderer {
  constructor(projectServer) {
    ...
    Object.assign(this.context.resources, {
      ...
      ssrTemplate: null,
      spaTemplate: null,
      errorTemplate: this.constructor.parseTemplate('Project Internal Server Error'),
    })
  }
  ...
  get assetsMapping() {
    ...
    const mapping = {}
    for (const legacyJsFile of Object.keys(legacyAssets)) {
  ...
  async loadResources(_fs = fs) {
    ...
    this.constructor.resourceMap.forEach(({ key, fileName, transform }) => {
      ...
      if (!_fs.existsSync(_path)) {
        ...
        //     logger.fatal(`Could not load Nuxt renderer, make sure to build for production: builder.build() with dev option set to false.`)
        //   }
        // }
